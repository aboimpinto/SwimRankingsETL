...
courses/events and other clubs. Source age categories and historical club records use the calendar year of each
race. The website profile ranking filter uses its selected comparison year,
matching CH/SR, and includes older PBs from swimmers in that selected cohort.
...
then commit and replay. After applying the log, call the protected website refresh endpoint and wait
for completion. Refresh invalidates the source cache and republishes prepared
profile responses; public profile reads do not rebuild them or expire by time. Standard future LENEX meet
