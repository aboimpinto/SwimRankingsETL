...
def main():
    ...
    try:
        for path in files:
            ...
            if meet_nation:
                cur.execute("SELECT 1 FROM countries WHERE code = %s LIMIT 1", (meet_nation,))
                if cur.fetchone() is None:
                    cur.execute("INSERT INTO countries (code, name, excluded) VALUES (%s, %s, false) ON CONFLICT (code) DO NOTHING", (meet_nation, meet_nation))
            ...
            if final_rows:
                ...
                imported_results += len(final_rows)
            ...
            cur.execute(
                ...
                ON CONFLICT (file_name) DO UPDATE SET
                    ...
                    error_msg=NULL
                """,
                (path.name, meet_id_str, meet_id_str)
            )

            imported_files += 1
            if imported_files % 50 == 0:
                print(f'FILES {imported_files} RESULTS_ROWS_ATTEMPTED {imported_results}')

        conn.commit()
        print(f'DONE files={imported_files} attempted_result_rows={imported_results}')
    except Exception:
        conn.rollback()
        raise
    finally:
