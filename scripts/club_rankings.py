#!/usr/bin/env python3
"""Complete official all-time Open club populations; retain every published row."""
import argparse, hashlib, json, re, time
from datetime import datetime, timezone
from pathlib import Path
from urllib.parse import parse_qs, urlparse
from club_record_baselines import canonical, digest, centiseconds, CLUB_ID, source_url, standard_events
...
def parse_page(html, url, course, gender, distance, stroke):
 from bs4 import BeautifulSoup
 s=BeautifulSoup(html,'html.parser')
 titles=[e.get_text(' ',strip=True) for e in s.select('.titleLeft,.titleRight,.titleCenter')]
 if 'Limmat Sharks Zuerich' not in titles or ('Women, Open' if gender=='F' else 'Men, Open') not in titles or ('Long Course (50m)' if course=='LCM' else 'Short Course (25m)') not in titles or 'Alltime' not in titles:
...
def validate_population(p):
 ...
 if p['key']!=f"{p['distance']}:{p['stroke']}:{p['course']}:{p['gender']}":raise ValueError('Invalid key')
...
def build_package(populations):
 ...
 expected={f"{e.split(':')[2]}:{e.split(':')[3]}:{course}:{gender}" for course in ('LCM','SCM') for gender in ('F','M') for e in standard_events(course)}
 ...
 return {'format':'swimrankings-club-rankings','version':1,'sha256':digest(payload),'payload':payload}

def apply(conn, package, commit=False):
 if build_package(package['payload']['populations'])!=package:raise ValueError('Invalid package/digest')
 ...
 try:
  ...
  with conn.cursor() as c:
   ...
   for p in package['payload']['populations']:
    ...
    if old and old[0]==p:report['unchanged']+=1;continue
 ...
 with sync_playwright() as pw:
  ...
  def fetch(url,path):
   ...
   if 'rankingList' not in result:
    (output/'unavailable.html').write_text(result)
    raise ValueError(f'Ranking unavailable at {url}; complete browser verification manually')
   path.write_text(result);path.chmod(0o600);time.sleep(2.0)
   return result
  for course in ('LCM','SCM'):
   for gender in ('F','M'):
    url=source_url(course,gender,'X_X');html=fetch(url,output/f'{course}-{gender}-catalog.html');soup=BeautifulSoup(html,'html.parser')
    events=[]
    for a in soup.select('a[href*="rankingClubId="]'):
     event=event_name(a.get_text(' ',strip=True))
     if event:events.append((event,a['href']))
    if len(events)<17 or len({e[0] for e in events})!=len(events):raise ValueError('Incomplete event catalog')
    for (distance,stroke),url in events:
     ...
     while total is None or offset<=total:
      ...
      h=fetch(u,output/f'{course}-{gender}-{distance}-{stroke}-{offset}.html');part=parse_page(h,u,course,gender,distance,stroke)
      ...
      print(f'{course} {gender} {distance} {stroke}: {len(rows)}/{total}',flush=True)
     p={'club_id':CLUB_ID,'key':f'{distance}:{stroke}:{course}:{gender}','course':course,'gender':gender,'distance':distance,'stroke':stroke,'total':total,'built':built,'captured_at':datetime.fromtimestamp(max((output/f'{course}-{gender}-{distance}-{stroke}-{v}.html').stat().st_mtime for v in range(1,total+1,25)),timezone.utc).isoformat(),'pages':pages,'rows':rows}
     validate_population(p);populations.append(p)
     target=output/'rankings.json';target.write_bytes(canonical(build_package(populations)));target.chmod(0o600)
  browser.close()
 print(json.dumps({'events':len(populations),'performances':sum(p['total'] for p in populations)}))

if __name__=='__main__':
 parser=argparse.ArgumentParser(description=__doc__);sub=parser.add_subparsers(dest='command',required=True)
 c=sub.add_parser('collect');c.add_argument('--output',type=Path,required=True);c.add_argument('--cdp',default='http://127.0.0.1:9334')
 ...
 if args.command=='collect':collect(args.output,args.cdp)
 else:
  import psycopg2
